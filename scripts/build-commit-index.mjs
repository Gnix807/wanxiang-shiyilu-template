 ...
 *   COMMIT_INDEX_REPO                  —— 覆盖 owner/repo，默认读取 src/config/build.json 或当前 GitHub remote
 */
...
const SCHEMA = 3;
const settings = JSON.parse(readFileSync(join('src', 'config', 'build.json'), 'utf8'));
const excludedAuthors = (settings.excludedAuthors || []).map(name => name.toLowerCase());
const includeAuthor = c => !excludedAuthors.some(name => c.author?.toLowerCase().includes(name));
function detectedRepo() {
  try {
...
const DEFAULT_REPO = process.env.GITHUB_REPOSITORY || settings.repository || detectedRepo();
...
async function main() {
  ...
  const base = existing ? existing.commits.filter(includeAuthor) : [];
    ...
    try {
      ...
      if (shallow) {
        ...
        for (const c of dropped) {
          if (cachedShas.has(c.sha) || process.env.COMMIT_INDEX_FETCH !== '1') continue;
          try {
  ...
  if (!entries || !entries.length) {
    ...
    let fresh = [];
    try {
      if (repo && process.env.COMMIT_INDEX_FETCH === '1') fresh = (await readApiCommits({ repo, sinceISO })).map(toEntry).filter(isMeaningful);
    } catch (e) {
  ...
  const temporaryPath = `${OUT_PATH}.${process.pid}.tmp`;
  writeFileSync(temporaryPath, JSON.stringify(payload), 'utf8');
  renameSync(temporaryPath, OUT_PATH);
  log(
