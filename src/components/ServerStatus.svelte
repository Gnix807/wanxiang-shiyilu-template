<script lang="ts">
  ...
  function showToast(msg: string) {
    ...
    t.innerHTML = [
      ...
      '    <div class="bg-red-500 text-white text-[10px] font-black flex items-center justify-center gap-1.5 px-2 py-1 rounded-sm mb-1.5"><svg class="w-3 h-3 stroke-[2.5]" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg> <span>服务器负载警告</span></div>',
      '    <div class="px-1 py-1 text-xs font-bold text-slate-700 dark:text-slate-200">' + msg + '</div>',
      '    <button class="js-toast-close mt-1 text-[10px] text-slate-400 hover:text-[#9e2a2b] font-bold" style="display:block;margin-left:auto">关闭</button>',
      '  </div>',
      '</div>'
    ].join('\n');
...
<div class="bg-white dark:bg-[#1c1b1a] border-4 border-[#1a1a1a] dark:border-[#e8e6e3] p-4 sm:p-6 shadow-[6px_6px_0px_0px_#1a1a1a] dark:shadow-[6px_6px_0px_0px_#e8e6e3] rounded-sm">
  <h2 class="text-xl sm:text-2xl font-black text-[#1a1a1a] dark:text-[#e8e6e3] tracking-wider mb-4 flex items-center gap-2">
    <svg class="w-5 h-5 text-[#9e2a2b] dark:text-[#d9534f] stroke-[2.2] shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg>
    <span>服务器带宽状态</span>
  </h2>

  <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 mb-4">
    <div class="bg-[#fbfbf9] dark:bg-[#242220] border-2 border-[#1a1a1a]/15 dark:border-[#e8e6e3]/20 rounded-sm px-3 py-2">
    ...
    <div class="bg-[#fbfbf9] dark:bg-[#242220] border-2 border-[#1a1a1a]/15 dark:border-[#e8e6e3]/20 rounded-sm px-3 py-2">
      ...
      <div class="text-lg font-black text-[#9e2a2b] dark:text-[#d9534f] font-mono">{avgDown.toFixed(2)} <span class="text-xs text-slate-400">MB/s</span></div>
    </div>
    <div class="bg-[#fbfbf9] dark:bg-[#242220] border-2 border-[#1a1a1a]/15 dark:border-[#e8e6e3]/20 rounded-sm px-3 py-2">
  ...
  <div class="bg-[#fbfbf9] dark:bg-[#242220] border-2 border-[#1a1a1a]/15 dark:border-[#e8e6e3]/20 rounded-sm p-2">
